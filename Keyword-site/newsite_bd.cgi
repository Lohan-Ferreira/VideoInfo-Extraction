...
message = form.getvalue("message", "-").lower()
olddict = form.getvalue("oldict")
run = form.getvalue("run")
sequences = form.getvalue("times")
...
if(scene_request is None):
	...
	scene_request = int(scene_request)
	bankid = int(bankid)
	videoid = int(videoid)
dicts=[]
...
if(already == 0):
	...
	tuplas=[]
	sparql.setQuery("""
	select  ?topic ?words WHERE{<http://videos/"""+str(videoid)+"""> <http://topics> ?topic . ?topic <http://keywords> ?words . }
	""")
	...
	for result in results["results"]["bindings"]:
		dicts.append(result["words"]["value"])


	#Guardando N Top-Keywords de cada cena para busca externa
	top_keywords = []
	n_top = 4
	for x in range(0,len(dicts)):
		separate = dicts[x].split(' ')
		text = ""
		if(len(separate)<n_top):
			n_top = len(separate)
		for y in range(0,n_top):
			text += separate[y] + ' '
...
vid.ontimeupdate = function(){
		...
		if(cena_atual != cena_buscada){
			...
			for(i = 0; i< relateds[cena_atual].length;i++){
				//if(parseFloat(relateds[cena_atual][i][1]) >= 0.9){
				...
				var t = document.createTextNode("Video "+videoid+" topico" + sceneid);
				submit.append(t);
...
function displayResults(results) {
  ...
  searchResults.insertAdjacentHTML("beforeend",
      `<img src="../wikipedia.png"  width="240px" height="180px">`);
...
Cenas Relacionadas: %s <br>
</body>

</html>
""" % (sequences,top_keywords,relateds,scene_request,bankid,dicts,sequences,top_keywords,bankid,videoid,message))
...
<gcse:searchresults-only gname="gsearch" webSearchQueryAddition=""></gcse:searchresults-only>
 ...
 <button class="collapsible">Wikipedia</button>
<div class="content" id="sdiv">
</div> 
