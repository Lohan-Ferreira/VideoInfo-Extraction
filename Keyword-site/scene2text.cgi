...
<video id="myvideo" width="320" height="240" controls>
  <source src="LMG.mp4" type="video/mp4">
...
<script>
function myFunction2(palavra)
{
	document.myform.message.value = palavra;
...
<p id="frase">Playback position:  </p>
...
vid.ontimeupdate = function(){
		...
		cena_atual++;
		var container = document.getElementById("frase");
   		var content = container.innerHTML;
    		//container.innerHTML= top_keywords[cena_atual]; 
...
function displayResults(results) {
  ...
  const searchResults = document.getElementById("frase");
...
<form name = "myform" method="post" action="scene2text.cgi">
...
""" % (tempo_inicio_cena,top_keywords,top_keywords,keyword,dicts,inicio_cena,keyword,top_keywords))
...
<button id="%s" onclick="myFunction(%s)">%s</button>
